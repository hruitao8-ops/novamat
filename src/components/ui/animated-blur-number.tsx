...
/**
 * <AnimateNumber> - 数字逐位过渡动画组件，灵感来自 iOS 的 `.contentTransition(.numericText())`
 * 和 motion.dev 的 <AnimateNumber>，加上：只有实际变化的数字才会模糊移动，未变化的数字不会动画。
 *
 * 工作原理：格式化后的值被拆分为字符并右对齐渲染，每个数位一个 <CharSlot>（按其距右侧的偏移键控，
 * 因此个位数/小数点在数字增长或缩小时保持稳定标识）。只有当某个字符与上一个不同时，
 * 该槽位才会挂载滑动+模糊层 - 因此 1199 -> 1200 只会动画 "99" -> "00"，"12" 保持稳定。
 * 滑动使用弹簧动画（--an-spring）实现 iOS 的稳定效果。
 *
 ...
 *
 * 单文件，零运行时依赖（仅需 React peer）- 样式内嵌并注入一次。
 */
